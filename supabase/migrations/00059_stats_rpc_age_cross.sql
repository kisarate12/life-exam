...
BEGIN
  ...
  by_character AS (
    ...
    FROM base GROUP BY character_name, world
  ),
  by_world AS (
    SELECT world, count(*) AS count FROM base GROUP BY world
  ),
  by_age AS (
    SELECT age_band, count(*) AS count FROM base WHERE age_band IS NOT NULL GROUP BY age_band
  ),
  -- 年代×キャラのクロス集計
  by_age_character AS (
    SELECT
    ...
    GROUP BY age_band, character_name, world
  ),
  totals AS (
  ...
  SELECT jsonb_build_object(
    ...
    'characters', (SELECT jsonb_agg(
      jsonb_build_object(
        'character_name', character_name, 'world', world, 'count', count,
        'financial_avg', financial_avg, 'human_avg', human_avg,
        'social_avg', social_avg, 'time_avg', time_avg, 'health_avg', health_avg
      ) ORDER BY count DESC
    ) FROM by_character),
    'worlds', (SELECT jsonb_agg(
      jsonb_build_object('world', world, 'count', count) ORDER BY count DESC
    ) FROM by_world),
    'age_bands', (SELECT jsonb_agg(
      jsonb_build_object('age_band', age_band, 'count', count) ORDER BY age_band
    ) FROM by_age),
    'age_characters', (SELECT jsonb_agg(
      jsonb_build_object(
        'age_band', age_band,
        'character_name', character_name, 'world', world, 'count', count,
        'financial_avg', financial_avg, 'human_avg', human_avg,
        'social_avg', social_avg, 'time_avg', time_avg, 'health_avg', health_avg
      ) ORDER BY age_band, count DESC
    ) FROM by_age_character)
  ) INTO result;
