-- 結果ページ用：指定受験の「平均点・順位」を全世代・同世代で取得するRPC
...
BEGIN
  ...
  IF v_total_score IS NULL THEN
    RETURN jsonb_build_object('total_avg_all', null, 'total_avg_same_gen', null, 'subjects', '[]'::jsonb);
  END IF;
...
COMMENT ON FUNCTION get_life_exam_comparison_stats(uuid) IS '結果ページ用。指定 attempt の全世代・同世代の平均点と科目別順位を返す。';
