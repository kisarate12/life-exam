-- 全国ベースサンプル：年齢・性別・大学・資産・所得・時間・健康・人間関係を実態に合わせて約5,500件投入。
-- 30〜50代多め、性別50:50、資産・所得は年代・性別実態、時間・健康・人間関係は平均〜やや下多め。
-- 大学は有名国公立・私立約21校 + 未記入20%。1大学50件以上確保。
-- 実行条件: auth.users に1件以上存在。00028 リセット後に実行する想定。
...
BEGIN
  ...
  WITH params AS (
    SELECT
      ...
      -- 年代別収入の中心（20代低め・30〜40代高め・50〜60代やや下）。性別は gen 内で加算。
  ...
  band_idx AS (
    SELECT
      (CASE
        WHEN band_r < 0.05 THEN 1 WHEN band_r < 0.10 THEN 2 WHEN band_r < 0.20 THEN 3
        WHEN band_r < 0.30 THEN 4 WHEN band_r < 0.42 THEN 5 WHEN band_r < 0.55 THEN 6
        WHEN band_r < 0.70 THEN 7 WHEN band_r < 0.85 THEN 8 ELSE 9
      END)::int AS bi,
  ...
  gen_with_total AS (
    SELECT
      rn, uid, band, gender, univ, s1, s2, s3, s4, s5,
  ...
  RAISE NOTICE '全国ベースサンプル 5,500 件を投入し、順位を再計算しました。';
END;
$$;
